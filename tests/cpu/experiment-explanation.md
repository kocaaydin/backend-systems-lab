...
1.  **"İşlemciyi Yoracak" Kod Eklendi (`Program.cs`)**
    *   API'ye `/experiments/cpu` adında yeni bir endpoint eklendi.
...
3.  **Otomasyon Scripti Hazırlandı (`tests/cpu/run-experiment-2-cpu.sh`)**
...
*   **Yüksek Yük (20+ RPS):** İşlemci kapasitesi (CPU core sayısı) yetersiz kalmaya başlar.
    ...
    *   **Belirtiler:** Yanıt süreleri (Latency) dramatik şekilde artar, Timeout hataları başlar ve hatta normalde çok hızlı çalışan `/health` gibi basit endpointler bile yanıt veremez hale gelebilir (çünkü onları işleyecek boş thread bulunamaz).
