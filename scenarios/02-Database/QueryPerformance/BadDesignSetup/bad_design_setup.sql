...
-- =============================================
-- 5. CASE LAB: BAD -> FIX -> GOOD (Tek dosya akisi)
-- =============================================
USE LabDb;
GO
...
/* =========================================================
   CASE 2: NVARCHAR(MAX) ile join (kotu) vs dogru tipte join
   BAD : CAST ile NVARCHAR(MAX) join
   FIX : INT -> INT join
========================================================= */
PRINT 'CASE 2 / BAD: NVARCHAR(MAX) join';
SELECT TOP 200
    o.Id,
    oi.ProductId,
    oi.Quantity
...
INNER JOIN OrderItems oi
    ON CAST(o.Id AS NVARCHAR(MAX)) = CAST(oi.OrderId AS NVARCHAR(MAX))
...
IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID('dbo.OrderItems')
      AND name = 'IX_OrderItems_OrderId'
)
BEGIN
...
/* =========================================================
   CASE 3: Heap etkisi (Orders)
   BAD : Heap tabloda point query
   FIX : Clustered index
========================================================= */
PRINT 'CASE 3 / BAD: Heap table point query';
SELECT TOP 1 *
FROM Orders
WHERE Id = 2500000;
GO
...
PRINT 'CASE 3 / GOOD: Clustered table point query';
SELECT TOP 1 *
FROM Orders
WHERE Id = 2500000;
GO
