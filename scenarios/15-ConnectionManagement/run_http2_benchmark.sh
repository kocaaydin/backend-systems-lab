...
echo "========================================================="
echo "🔬 HTTP/2 MULTIPLEXING TESTİ (TLS)"
...
' "$summary_json" 2>/dev/null || echo "0,0,0,0,0")
    ...
    echo "$scenario_name,$vus,$host_port,$avg,$p50,$p95,$p99,$rps,$timewait_diff" >> "$RESULTS_DIR/report_http2.csv"
}

echo "Senaryo,VU,Port,Ort(ms),P50(ms),P95(ms),P99(ms),RPS,Eklenen_TIME_WAIT" > "$RESULTS_DIR/report_http2.csv"

for vu in "${CONCURRENCY_LEVELS[@]}"; do
    run_scenario "HTTP2MultiplexOn" "k6/http2_multiplex_on.js" $vu "15021" "15022"
    sleep 5
done

echo ""
echo "========================================================="
echo "📊 HTTP/2 TEST SONUÇ RAPORU"
echo "========================================================="
column -s, -t "$RESULTS_DIR/report_http2.csv"
